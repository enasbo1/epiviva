...
import {DistributeAffectedObject, DistributeObject} from "./distributeObject";

@Injectable({
  providedIn: 'root'
})
export class DistributeModelService extends RequestService{
  post_distribute(content:DistributeObject, errorEvent?:EventEmitter<HttpErrorResponse>):Observable<object>{
  ...
  get_one_distribute(number:number|bigint):Observable<DistributeObject[]>{
    return (this.get_one('distribute',number) as Observable<DistributeObject[]>);

  }

  update_distribute(content:DistributeObject):Observable<object>{
